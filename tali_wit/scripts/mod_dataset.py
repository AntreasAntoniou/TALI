...
def gen_mod_data(
    ...
    with tqdm.tqdm(total=len(dataset)) as pbar:
        for example in dataset:
            ...
            example["youtube_subtitle_text"] = example[
                "youtube_subtitle_text"
            ].replace("/data/datasets/tali-wit-2-1-buckets/", "/data/")
            pbar.update(1)
            yield example
...
def gen_test():
    return gen_mod_data(
        set_name="test", tali_dataset_dir="/data_fast/tali-v-3-4/"
    )
...
def gen_val():
    return gen_mod_data(
        set_name="val",
        tali_dataset_dir="/data_large/datasets/tali-wit-2-1-buckets/",
    )


def gen_train():
    return gen_mod_data(
        set_name="train", tali_dataset_dir="/data_fast/tali-v-3-4/"
    )


if __name__ == "__main__":
    ds = Dataset.from_generator(
        gen_val, cache_dir="/data_fast/tali-v-3-4/val/cache"
    )
