...
class TALIModelDataParallel(nn.Module):
    def __init__(
        ...
        self.model = TALIModule(
            ...
            multi_modality_config=multi_modality_config,
        )
    ...
    def forward(
        ...
        output_dict = self.model(
            x=x, restrict_source_modality=restrict_source_modality
        )
                ...
                for sub_modality_a_name, sub_modality_a in modality_a.items():
                    for (
                        ...
                        similarity_dict |= get_similarities(
                            ...
                            sub_modality_a["projection_output"],
                            sub_modality_b["projection_output"],
                            logit_scale=self.model.module.logit_scales[
                                f"{modality_a_name}_to_{modality_b_name}"
                            ],
                            return_loss=return_loss,
                        )
